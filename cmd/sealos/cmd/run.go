...
create cluster to your baremetal server, appoint the iplist:
  ...
  multi image:
    sealos run nebstudio/kubernetes:v1.24.0 calico:v3.24.1 \
  ...
  Different InfraSSH port numbers exist:
	sealos run nebstudio/kubernetes:v1.24.0 --masters 192.168.0.2,192.168.0.3:23,192.168.0.4:24 \
  ...
  Single kubernetes cluster:
	sealos run nebstudio/kubernetes:v1.24.0 --single
...
func newRunCmd() *cobra.Command {
	...
	var runCmd = &cobra.Command{
		...
		Long:    `sealos run nebstudio/kubernetes:v1.24.0 --masters [arg] --nodes [arg]`,
		Example: exampleRun,
